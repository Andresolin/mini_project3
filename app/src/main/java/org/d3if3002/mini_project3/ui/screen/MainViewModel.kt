...
class MainViewModel : ViewModel() {
    ...
    fun retrieveData(userId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                data.value = CityApi.service.getCity(userId)
    ...
    fun saveData(userId: String, city: String, country: String, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val result = CityApi.service.postCity(
                    userId,
    ...
    fun deleteCity(userId: String, id: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val result = CityApi.service.deleteCity(userId, id)
                if (result.status == "success") {
                    retrieveData(userId)
                } else {
                    throw Exception(result.message)
